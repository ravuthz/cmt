...
<script language="javascript" type="text/javascript">
	function getpackage(check){
		service = creditnote.selservice.value;		
		
		if(service == ""){
			alert("Please select Service");
			creditnote.selservice.focus();
			return;
		}else{
	...
	function showReport(){
		st = creditnote.st.value;
		et = creditnote.et.value;
		where = "0 ";		
		if(creditnote.selservice.selectedIndex <1){
			alert("Please select service");
		}else{
			if((st == "") || (et == "")){
				...
				for(i=0; i< creditnote.package.length; i++){
					if(creditnote.package[i].checked == true)
						where += ", " + creditnote.pack[i].value;
				}
				var loading;
		loading = "<table width='100%' height='100%' border=0 cellspacing=0 cellpadding=100><tr><td align=center><p><img src='./images/loading.gif' border=0><br><b>Please wait a moment...</b></p></td></tr></table>";
				document.getElementById("d-creditnote").innerHTML = loading;
				url = "./php/ajax_creditnote.php?st="+st+"&et="+et+"&where="+where+"&mt="+new Date().getTime();
				getTranDetail(url, "d-creditnote");
			}
		}
	}
</script>
	...
	<tr>		
		<td valign="top" align="left">
			<table border="0" cellpadding="2" cellspacing="0" class="formbg" align="left">
				<tr>
					<td align="left" class="formtitle"><b>&nbsp;CREDIT NOTE REPORT</b></td>
					<td align="right"></td>
				</tr> 
				<tr>
					<td colspan="2">
						<form name="creditnote" onSubmit="return false;">
							...
							<tr>
								...
								<td align="left">
									<input type="text" tabindex="1" name="st" class="boxenabled" size="27" maxlength="30" value="<?php print date("Y-m-d"); ?>" onKeyUp="DateFormat(this,this.value,event,false,'2')" onBlur="DateFormat(this,this.value,event,true,'2')" />
											<button class="invisibleButtons" onClick="window.open( './javascript/calendar.html?creditnote|st', '', 'width=200,height=220,top=250,left=350');">
							...
							<tr>
								...
								<td align="left">
									<input type="text" tabindex="2" name="et" class="boxenabled" size="27" maxlength="30" value="<?php print date("Y-m-d"); ?>" onKeyUp="DateFormat(this,this.value,event,false,'2')" onBlur="DateFormat(this,this.value,event,true,'2')" />
											<button class="invisibleButtons" onClick="window.open( './javascript/calendar.html?creditnote|et', '', 'width=200,height=220,top=250,left=350');">
												<img src='./images/b_calendar.png' alt='View Calendar' align="middle" border="0">
											</button>&nbsp;(yyyy-mm-dd) 
								</td>
							</tr>	
