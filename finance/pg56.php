...
<script language="JavaScript" type="text/javascript">
	...
	function ChangeInvoice(index){
		if(parseFloat(fcreditinvoice.cbalance.value) > 0){		
			if(index > 0){
	...
	function checkAmount(balance,amount ){		
	...
	function submitForm(){
		balance = fcreditinvoice.cbalance;
		...
		if(fcreditinvoice.InvoiceID.selectedIndex < 1){
			...
			Amount.focus();
			return;
		}else if(!checkAmount(Trim(balance.value), Trim(Amount.value))){
			alert("Unbale to settle more than your current balance");
			Amount.focus();
			return;
...
<?php
	...
	if(!empty($smt) && isset($smt) && ($smt == "save56")){
		$Finance = new Payment();
		$Audit = new Audit();		
		...
		# Transaction mode id:15 ==> Write off bad debt
		$TransactionMode = 10;
		$selPaymentID = 3;
		if($DrawerID == 0){
			
			$retOut = $myinfo->warning("Failed to write off invoice ".$InvoiceID.". No cash drawer open for cashier ".$user["FullName"]);
		}else{		
			...
			if($retOut){
				$Description = "Write off Invoice No. $InvoiceID amount ".FormatCurrency($Amount)."- $Description";
						...
						<tr>
							...
							<td valign="top" align="left">
								<table border="0" cellpadding="2" cellspacing="0" class="formbg" align="left" width="100%">
									<tr>
										<td align="left" class="formtitle"><b>SETTLE INVOICE WITH BALANCE</b></td>
										<td align="right">&nbsp;
									...
									<tr>
										<td colspan="2">
											<form name="fcreditinvoice" method="get" action="./" onSubmit="return false;">
												<table border="0" cellpadding="3" cellspacing="0" align="center" width="100%" bgcolor="#feeac2">													
													<tr>
														...
														<td align="left">
															<input type="text" name="cbalance" class="boxdisabled" value="<?php print $cbalance; ?>" />
														</td>
													</tr>
													<tr>
														<td colspan="2">&nbsp;</td>
													...
													<tr>
														<td align="left" nowrap="nowrap">settle amount:</td>
											...
											<input type="hidden" name="pg" value="56" />
											<input type="hidden" name="smt" value="save56" />
											</form>	
										</td>
									</tr>			
								</table>
