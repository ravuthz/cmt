...
$sql = "SELECT j.JobID, j.AccID, j.CurrentStatusID, j.NewStatusID, j.SubmitDate, j.Incoming, j.Outgoing, j.SubmitDate,
					...
					j.SWComment, j.OpDone, j.OPDoneDate, j.OPComment, j.MDFDoneDate, j.MDFComment, j.ISPDoneDate, j.ISPComment, j.Incoming, a.CustID, a.UserName, j.csDate					 
...
//											$StatusID = intval($result['StatusID']);
										...
										//	$linkAccount = "<a href='./?CustomerID=".$CustID."&AccountID=".$AccID."&pg=91'>".$UserName."</a>";
											...
											//MDF
											$retOut .= '<td class="'.$style.'" align="left">'.formatDate($MDFDoneDate, 7).'</td>';
											$retOut .= '<td class="'.$style.'" align="left">'.$MDFComment.'</td>';
											//ISP
											$retOut .= '<td class="'.$style.'" align="left">'.formatDate($ISPDoneDate, 7).'</td>';
