...
<?php
	...
	$retOutH='<br><br><table border="1" cellpadding="3" cellspacing="0" align="center" width="100%" height="100%" id="audit3" class="sortable" style="border-collapse:collapse" bordercolor="#cccccc">
							...
							<thead>																									
								...
								<th align="center" width="5%" style="border-left:1px solid #999999; border-top:1px solid #999999;">Credit Amount</th>
	...
	$sql = "
			...
			swstatus ,case when (limitamount/100)*$w<=(Sum(IsNull(B.Amount,0))-Sum(IsNull(Ab.Credit,0))-sum(IsNull(adv.Paymentamount,0))) then
			 'Close' else 'Rec' end state,limitamount from tblCustomer
			 C Inner Join tblCustproduct p on C.CustID=P.CustID Inner Join tblTarPackage T on P.packageID=T.packageID Inner Join #billingtmp B on 
			 P.accID=B.accID Inner Join tblCreditLimit cl on p.accID=cl.accID and (isnull(cl.limitamount,0)<>0) Left Join tblCustCashDrawer bkn on P.accID=bkn.AcctID 
			 and bkn.IsSubmitted=0 and (bkn.IsRollBack is Null or bkn.IsRollBack=0) and 
			 Bkn.TransactionModeID in (4) Left Join tblCustCashDrawer bki on P.accID=bki.AcctID and bki.IsSubmitted=0 and (bki.IsRollBack is Null or bki.IsRollBack=0) 
			 and Bki.TransactionModeID in (5) Left Join tblCustCashDrawer bkm on P.accID=bkm.AcctID and bkm.IsSubmitted=0 and (bkm.IsRollBack is Null or 
			 bkm.IsRollBack=0) and Bkm.TransactionModeID in (6)	Left Join tblCustCashDrawer adv on P.accID=adv.AcctID and adv.IsSubmitted=0 and	(adv.IsRollBack is Null 
			 or adv.IsRollBack=0) and adv.TransactionModeID in (3) Left Join tblAccDeposit Ad on Ad.AccID=P.accID Left Join tblAccountBalance Ab on P.accID=Ab.accID 
			 Left Join (tblException E Inner Join tblExceptionDetail Ed on E.accID=Ed.accID and Remark='Exception') on p.accID=E.accID Left Join 
			 tblSwitchUsageStatusCreditLimit du on p.accID=du.accID and b.BillingCycleID=du.cycleID group by 
			  ...
			  ,du.swstatus,curbook,limitamount having (limitamount/100)*$w
			  <=(Sum(IsNull(B.Amount,0))-Sum(IsNull(Ab.Credit,0))-sum(IsNull(adv.Paymentamount,0))) or du.swstatus in (0,1,2)) V1 Order by Remark 
			  asc,swstatus,Package,payment desc,PHone 
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){	
			...
			/*if((((intval($swstatus)==0) || (intval($swstatus)==1) || (intval($swstatus)==2)) && ($b<=$curbook) && $b!=0) || ($state=='Rec') || ($swstatus==0 && $state=='Close'))  
			{
				$rtype="disabled";
			}
			else $rtype="";*/
