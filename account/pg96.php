...
<?php
	...
	# =============== Get customer header =====================
		...
		if(!empty($smt) && isset($smt) && ($smt == "save96")){		
			$Audit = new Audit();
			$txtAmount = FixQuotes($txtAmount);
			$IsPercent = FixQuotes($IsPercent);
			$txtDescription = FixQuotes($txtDescription);
			
			$now = date("Y/M/d H:i:s");
			$Operator = $user["FullName"];
			$CreditType = 1; # One time credit
			$InvoiceItemID = 12; # one time credit
			$error = false;
			...
			if(!$error){			
				...
				if($mydb->sql_query($sql)){
					...
					$sql = "INSERT INTO tblHisAdjustment(CustomerID, AccountID, Amount, DisType, DiscountDate, Operator, Description)
									VALUES($CustomerID, $AccountID, $txtAmount, 1, GetDate(), '".$user['FullName']."', '".$txtDescription."')";
					if($mydb->sql_query($sql)){
						$Description = "Add credit one time adjustment amount ".FormatCurrency($txtAmount).". $txtDescription";
						$Audit->AddAudit($CustomerID, $AccountID, "One time credit adjustment", $Description, $user['FullName'], 1, 12);
					...
					$error = $mydb->sql_error();
					$retOut = $myinfo->error("Failed to add credit adjustment.", $error['message']);				
				}
			}												
		}				
?>
	...
	<tr>
		...
		<td valign="top" align="left">
			<table border="0" cellpadding="2" cellspacing="0" class="formbg" align="left" width="100%">
									<tr>
									 	<td align="left" class="formtitle"><b>ONE TIME CREDIT ADJUSTMENT
										...
										<td valign="top" colspan="2">
											...
											<table border="0" cellpadding="4" cellspacing="0" width="100%" height="100%" class="formbody" bordercolor="#aaaaaa">																								
												...
												<tr>
													...
													<td align="left">
														<input type="text" name="txtDescription" tabindex="1" class="boxenabled" size="70" maxlength="100" />
													</td>
												</tr>
												<tr><td colspan="2">&nbsp;</td></tr>								
													...
													<td align="center" colspan="2">
														...
														<input type="submit" tabindex="3" name="btnSubmit" value="Submit" class="button" onClick="submitForm();" />						
													</td>
												 </tr>
												 <?php
											...
											<input type="hidden" name="pg" value="96" />
											<input type="hidden" name="smt" value="save96" />
											</form>
										</td>
									</tr>							
								</table>						
