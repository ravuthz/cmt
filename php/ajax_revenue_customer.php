...
<?php
	...
	$sql = " SELECT c.CustID, c.CustName, r.InvoiceTypeID, sum(r.TotalInvoice) 'Inv', Sum(r.Duration) 'duration', 
									SUM(r.OtherCharge) 'Other', MIN(i.TransStartDate) 'TransStartDate', MAX(i.TransEndDate) 'TransEndDate',
	...
	$sql .= " GROUP BY r.InvoiceTypeID, c.CustID, c.CustName 
						ORDER BY r.InvoiceTypeID,  c.CustName,c.CustID";
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){																															
			...
			if(($iLoop % 2) == 0)											
				...
				$style = "row2";
			$Cust = "<a href='./ajax_revenue_customer_detail.php?cid=".$cid."&custid=".$CustID."&custname=".$CustName."&tid=".$tid."&ct=".$ct."&tt=".$itype."&serid=".$sid."&st=".$st."&sn=".$sn."&tid=".$InvoiceTypeID."' target='_blank'>".$CustID."</a>";
			$retOut .= '<tr>';																																													
			...
			$retOut .= '<td class="'.$style.'" align="right" style="border-left:1px dotted #999999; border-top:1px dotted #999999;">'.$Cust.'</td>';
