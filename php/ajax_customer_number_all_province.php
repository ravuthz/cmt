...
function generateReport($cid, $pid, $tid, $ct, $pt, $tt, $sid, $pack){
	global $mydb;
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){																															
			...
			if(($iLoop % 2) == 0)											
				...
				$style = "row2";
			$Inv = "<a href='../report/revenue_detail_group_loc.php?sid=".$sid."&st=".$cid."&it=".$InvoiceType."&pid=".$pid."&pt=".$pt."&tt=".$tt."&pack=".$pack."' target='_blank'>".$Inv."</a>";
			$retOut .= '<tr>';																																													
		...
		$TotalInv = "<a href='../report/revenue_detail_group_loc.php?sid=".$sid."&st=".$cid."&it=All&pid=".$pid."&pt=".$pt."&tt=".$tt."&pack=".$pack."' target='_blank'>".$TotalInv."</a>";
				...
				if($que2 = $mydb->sql_query($sql2)){
					while($result2 = $mydb->sql_fetchrow($que2)){
						...
						$GrpPackage = $result2['GrpPackage'];
							$retfun .= "<tr><td>";
							$retfun .= generateReport($cid, $GrpPackage, $tid, $ct, $GrpPackage, $tt, $GrpServiceName, $where);
							$retfun .= "</td></tr>";
					}
				}
	$retfun .= "<tr><td>";
	$retfun .= generateReport($cid, "GRAND TOTAL", $tid, $ct, "GRAND TOTAL", $tt, $GrpServiceName, $where);
	$retfun .= "</td></tr>";
	$retfun .= "</table>";
