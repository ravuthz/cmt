...
<?php
	...
	if(isset($ext) && (!empty($ext)) && ($ext != "") && ($ext == "ap")){			
		...
		$sql = "INSERT INTO tblCustGuarrantor(CustID, GuarrantorName, Salutation, DOB, Address, KhanID, SangkatID, CityID, CountryID, 
								NationalityID, OccupationID, Phone, Email, IdentityMode, IdentityData, SignupDate,Street)
						VALUES('".$CustomerID."', '".$txtGarrentorName."', '".$selGuaSalutation."', '".$txtGuaDOB."', 
									 ...
									 '".$GuaEmail."', '".$selGuaDuplicateID."', '".$txtGuaDuplicate."', '".$now."','".$txtGuaStreet."')";
		...
		if($mydb->sql_query($sql, true)){
			...
			$sql = "INSERT INTO tblCustDesignate(CustID, DesignateName, Salutation, DOB, Address, KhanID, SangkatID, CityID, CountryID, 
						...
						VALUES('".$CustomerID."', '".$txtDesignateName."', '".$selDesSalutation."', '".$txtDesDOB."', 
									 ...
									 '".$DesEmail."', '".$selDesDuplicateID."', '".$txtDesDuplicate."', '".$now."','".$txtDesStreet."')";
			...
			if($mydb->sql_query($sql, true)){
				...
				$sql = "INSERT INTO tblCustContact(CustID, ContactName, Salutation, DOB, Address, KhanID, SangkatID, CityID, CountryID, 
						...
						VALUES('".$CustomerID."', '".$txtContactName."', '".$selConSalutation."', '".$txtConDOB."', 
									 ...
									 '".$ConEmail."', '".$selConDuplicateID."', '".$txtConDuplicate."', '".$now."','".$txtConStreet."')";
				...
				if($mydb->sql_query($sql, true)){
					...
					$sql = "INSERT INTO tblCustomer(CustID, CustName, CustTypeID, VATNumber, IsVATException, RegisteredDate, IdentityData,
										...
										VALUES('".$CustomerID."', '".$txtCustomerName."', '".$radCustType."', '".$txtVATNumber."',
													 ...
													 '".$selCusCity."', '".$selCusCountry."', '0', '".$selBusinessType."', '".$txtCusEmail."','".$txtCusStreet."','Adding New Customer From system')";
	...
	if($IsAddProduct){			
			...
			$sql = "INSERT INTO tblCustAddress(CustID, IsBillingAddress, Address, SangkatID, KhanID, CityID, CountryID, AccID)
							VALUES('".$CustomerID."', 0, '".$txtIntAddress.", ".$txtIntStreet."', '".$selIntSangkat."', '".$selIntKhan."', '".$selIntCity."',
			...
			if($mydb->sql_query($sql, true)){
				...
				$sql = "INSERT INTO tblCustAddress(CustID, IsBillingAddress, Address, SangkatID, KhanID, CityID, CountryID, AccID)
								VALUES('".$CustomerID."', 1, '".$txtBilAddress.", ".$txtBilStreet."', '".$selBilSangkat."', '".$selBilKhan."', '".$selBilCity."',
				...
				if($mydb->sql_query($sql, true)){
					...
					if($radConnection == 2){
						$sql = "INSERT INTO tblCustAddress(CustID, IsBillingAddress, Address, SangkatID, KhanID, CityID, CountryID, AccID)
								VALUES('".$CustomerID."', 2, '".$txtLeaAddress.", ".$txtLeaStreet."', '".$selLeaSangkat."', '".$selLeaKhan."', '".$selLeaCity."',
					...
					{	
						$sql = "INSERT INTO tblTrackAccount(AccID, PackageID, UserName, Password, StatusID, RegDate, 
											RegBy, SubscriptionName, Track,Context,
											...
											VALUES('".$AccountID."', '".$PackageID."', '".$txtAccountName."', '".$Password."', 
														 '".$StatusID."', '".$now."', '".$Operator."', '".$SubscriptionName."',
					...
					$sql = "INSERT INTO tblCustProduct(AccID, CustID, PackageID, UserName, Password, SalePersonID, StatusID, SetupDate, 
										CreatedBy, SubscriptionName, NoBillRun, Score, StationID, IsNCDeposit, IsICDeposit, IsMFDeposit, InvoiceTypeID, MessengerID, 	
										BillingEmail,Track,MaxTrackID,bHouse,bStreet,bSangkatID,bKhanID,bCityID,bCountryID,VATnumber,IsVATException,GrpInvID) 
										VALUES('".$AccountID."', '".$CustomerID."', '".$PackageID."', '".$txtAccountName."', '".$Password."', 
													 ...
													 "', '".$SelMessengerID."', '".$txtBilEmail."','New',".$TrackID.",'".$txtBilAddress."', '".$txtBilStreet."', '".
					...
					if($mydb->sql_query($sql, true)){
						...
						$audit->AddAudit($CustomerID, $AccountID, $title, $comment, $Operator, 1, 1);
							...
							if($retOut){
								...
								$retOut = $Finance->CreateInvoiceDetail($CustomerID, $AccountID, $InvoiceID, $ItemID, $RegistrationFee, 0, $vatRegistrationFee, $TrackID);
								if($retOut){
									//
									...
									$retOut = $Finance->CreateInvoiceDetail($CustomerID, $AccountID, $InvoiceID, $ItemID, $ConfigurationFee, 0, $vatConfigurationFee, $TrackID);
									if($retOut){
										//
										//	add cpe fee
										//
										$ItemID = getInvoiceItem("CPE Fee");
										$retOut = $Finance->CreateInvoiceDetail($CustomerID, $AccountID, $InvoiceID, $ItemID, $CPEFee, 0, $vatCPEFee, $TrackID);
										if($retOut){
											//
											//	add isdn fee
											//
											$ItemID = getInvoiceItem("ISDN Fee");
											$retOut = $Finance->CreateInvoiceDetail($CustomerID, $AccountID, $InvoiceID, $ItemID, $ISDNFee, 0, $vatISDNFee, $TrackID);
											if($retOut){
												//
												//	add special number									
												//	
												...
												$retOut = $Finance->CreateInvoiceDetail($CustomerID, $AccountID, $InvoiceID, $ItemID, $SPNFee, 0, $vatSPNFee, $TrackID);
												if($retOut){
													//create invoice
													//	
													//
...
<script language="javascript">
	...
	function ValidAccount(retDiv){
		...
		getServiceID("./php/ajax_getServiceID.php?PackageID=" + PackageID +"&mt=" + new Date().getTime(),fsignupcust1.sid);
                                        ...
                                        <table border="0" cellpadding="2" cellspacing="0" width="100%" height="100%" bgcolor="#feeac2" align="left">													
                                            ...
                                            <tr>
                                                ...
                                                <td align="left">
                                                    ...
                                                    <input type="text" name="UserName" id="UserName" value="<?php print $UserName;?>"  class="boxenabled" tabindex="75" size="21" onblur="ValidAccount('dUserName');" maxlength="100" /><img src="./images/required.gif" border="0" /><span style="display:none" id="dUserName" class="error"></span>
                                                </td>
                                            </tr>
                                            <tr>   <!--===============Group Invoice================-->
                                                        ...
                                                        <?php
                                                            ...
                                                            if($que){
                                                                while($rst = $mydb->sql_fetchrow($que)){	
																	...
																	if($ID==1)
																	{																	
																		print "<option value='".$ID."' checked>".$groupinvoice."</option>";
																	}
																	else
																	{																	
