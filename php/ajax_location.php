...
$d = $_GET['d'];
	switch ($t){
		...
		case 7: # get AccID by UserName
			$sql = "select case when statusid = 1 then 'Active_' when statusid = 4 then 'Close_' when statusID = 0 then 'InActive_' when statusid = 3 then 'Demand_' end + UserName + '_' +SubscriptionName + '_' + convert(varchar,AccID) 'name', AccID 'id' from tblCustProduct Where UserName like '%".$q."%' order by name";
		break;
		case 8: # get AccID by SubScriptionName
			$sql = "select case when statusid = 1 then 'Active_' when statusid = 4 then 'Close_' when statusID = 0 then 'InActive_' when statusid = 3 then 'Demand_' end + UserName + '_' +SubscriptionName + '_' + convert(varchar,AccID) 'name', AccID 'id' from tblCustProduct Where subscriptionname like '%".$q."%' order by name";
		break;
		case 9: # get AccID by CustomerName
			$sql = "select case when statusid = 1 then 'Active_' when statusid = 4 then 'Close_' when statusID = 0 then 'InActive_' when statusid = 3 then 'Demand_' end + UserName + '_' +SubscriptionName + '_' + convert(varchar,AccID) 'name', AccID 'id' from tblCustProduct Where CustID in (select CustID from tblCustomer where CustName like '%".$q."%') order by name";
		break;
		case 10: # get AccID by AccID
			$sql = "select case when statusid = 1 then 'Active_' when statusid = 4 then 'Close_' when statusID = 0 then 'InActive_' when statusid = 3 then 'Demand_' end + UserName + '_' +SubscriptionName + '_' + convert(varchar,AccID) 'name', AccID 'id' from tblCustProduct Where AccID=$q order by name";
		break;
