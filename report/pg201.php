							...
							<tbody>
								<?php
										$sql = "SELECT j.JobID, j.AccID, j.CurrentStatusID, j.NewStatusID, j.SubmitDate, 
															...
															j.IncomingNat, j.OutgoingLoc, j.OutgoingNat, j.Other, j.ISPComment, j.OPComment, a.CustID, a.UserName,a.SubscriptionName 'SubName', tp.tarName
														FROM tblJobAcctStatus j, tblCustProduct a, tblTarPackage tp
														WHERE j.AccID = a.AccID and a.Packageid = tp.Packageid and ((j.IsConfirm = 1 or j.ISPDone = 1) and (j.OpDone = 1)) and
															 j.MDFDone = 1 and j.IsDone = 0 
