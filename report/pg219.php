							...
							<tbody>
								<?php
										$sql = "SELECT j.JobID, j.AccID, j.CurrentStatusID, j.NewStatusID, j.SubmitDate, 
															...
															j.IncomingNat, j.OutgoingLoc, j.OutgoingNat, j.Other,	a.CustID, a.UserName, a.Password,a.SubscriptionName 'SubName', p.tarName
														FROM tblJobAcctStatus j(nolock), tblCustProduct a(nolock), tblTarPackage p(nolock)
														WHERE j.AccID = a.AccID and a.PackageID = p.PackageID and p.ServiceID in(1, 3, 8)
															and j.ISPDone = 0 and IsDone = 0 ORDER BY j.EffectiveDate,  j.JobID";
...
//											$StatusID = intval($result['StatusID']);
											...
											print '<td class="'.$style.'" align="left">'.$Password.'</td>';
