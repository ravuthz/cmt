...
<?php
	...
	$sql = "select * from dbo.fn_latelypaid_1m_isp_request_u_f('".$cid."','".$where."') where status<>'Closed' order by ty,exception desc,remark desc ,state,del,pdate,phone,CustName";
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){	
			...
			if ($Exception=='DELAY' && (($Expire>=$pd) && (is_null($pd)==0)))  
			{
				$rtype = "disabled";
				$Rec='Before delay';
			}			
			...
			if (intval($taf)==1)
			{
				$Done = "<input type=Button Onclick=\"Javascript:doIt(".$InvoiceID.",'".$phone."','".$in.'1'."','".$in."',".$AccID.",'".$dnow."');\" ".$rtype." name=\"btnSubmit\" value=\"Done\" style=\"font-size:8px;\" ID='".$in.'1'."' >";
			}
			else
			{
				$Done = "<input type=Button Onclick=\"Javascript:doAf(".$InvoiceID.",'".$phone."','".$in.'1'."','".$in."',".$AccID.",'".$dnow."');\" ".$rtype." name=\"btnSubmit\" value=\"Done\" style=\"font-size:8px;\" ID='".$in.'1'."' >";
			}
