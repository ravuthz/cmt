...
<?php
   ...
   // error_reporting(E_ALL);
	...
	function CheckUserNameExisting($UserName){
		global $mydb,$myinfo;
		$test=true;
		$sql = "Select * FROM tblCustProduct WHERE username='".stripcslashes(trim($UserName))."' and username not in (select AccName from tblException)";
		$query= $mydb->sql_query($sql);
		$numrow=intval($mydb->sql_numrows($query));
		if($numrow>0){
			$test=false;	
	...
	function CheckUserNameExistingDetail($UserName){
		global $mydb,$myinfo;
		$test=false;
		$sql = "Select * FROM tblCustProduct WHERE username='".stripcslashes(trim($UserName))."' and username not in (select AccName from tblException)";
		$query= $mydb->sql_query($sql);
...
//	
	...
	function HasExceptionDetail($accid){
		global $mydb,$myinfo;
		$blresult=false;
		$sql="Select * from tblExceptionDetail WHERE AccID='".$accid."'";
		$result=$mydb->sql_query($sql);
		$numrow=$mydb->sql_numrows($result);
		if($numrow>0){
			$blresult=true;
