...
require_once("functions.php");
	...
	class Invoice{
		...
		//
		function CreateInvoiceDetail($CustomerID, $AccountID, $InvoiceID, $BillItemID, $Amount, $BilledUnit = 0, $IsVAT = 0, $TrackID){
			global $mydb, $myinfo;
			...
			//$Amount = floatval(FormatCurrency($Amount, "", 1, 2));
				...
				$sql = "INSERT INTO tblCustomerInvoiceDetail(InvoiceID, SequenceNo, BillItemID, Description, 
															BillUnit, Amount, IsVAT, AccID, CustID, TrackID) 
								VALUES(".$InvoiceID.", ".$InvoiceSequence.", ".$BillItemID.", '".$Description."', 
							".$BilledUnit.", ".number_format($Amount, 2, '.', '').", ".$IsVAT.", ".$AccountID.", ".$CustomerID.",".$TrackID.")";

			if($mydb->sql_query($sql)){
				if($IsVAT == 1)
				{
					$VATRate = intval(getConfigue('VAT Rate'));
					$VATAmount = doubleval(($Amount * $VATRate) / 100); 
					
					$sql = "INSERT INTO tblCustomerInvoiceDetail(InvoiceID, SequenceNo, BillItemID, Description, 
																BillUnit, Amount, IsVAT, AccID, CustID, TrackID) 
									VALUES(".$InvoiceID.", 30, 5, 'VAT 10%', 
								".$BilledUnit.", ".number_format($VATAmount, 2, '.', '').", 0, ".$AccountID.", ".$CustomerID.", ".$TrackID.")";
					$mydb->sql_query($sql);
				}
		...
		function CreateInvoiceMultiItem($CustomerID, $AccountID, $InvoiceID, $TransactionStart = "", $TransactionEnd = "", $TrackID = 0){
			global $mydb, $myinfo;
			...
			$sql = "SELECT SUM(Amount) as InvoiceAmount FROM tblCustomerInvoiceDetail WHERE InvoiceID = $InvoiceID and BillItemID<>5";
			$que = $mydb->sql_query($sql);
			if($result = $mydb->sql_fetchrow())
			...
			$sql = "SELECT SUM(Amount) as InvoiceAmountVAT FROM tblCustomerInvoiceDetail WHERE InvoiceID = $InvoiceID AND IsVAT = 1 and BillItemID<>5";
			$que = $mydb->sql_query($sql);
			if($result = $mydb->sql_fetchrow())
			...
			#	Create invoice
			$sql = "INSERT INTO tblCustomerInvoice(InvoiceID, CustID, IssueDate, DueDate, InvoiceAmount, VATAmount, UnpaidAmount, BroughtForwardBalance, OriginalUnpaidAmount, NetAmount, AccID, BillingCycleID, TransStartDate, TransEndDate, InvoiceType, TrackID) 
			VALUES ($InvoiceID, $CustomerID, '".$today."', '".$InvoiceDeuDate."', ".number_format($TotalAmount, 2, '.', '').", ".number_format($VATAmount, 2, '.', '').", ".number_format($UnpaidAmount, 2, '.', '').", ".number_format($BalanceBroadforward, 2, '.', '').", ".number_format($UnpaidAmount, 2, '.', '').", ".number_format($InvoiceAmount, 2, '.', '').", $AccountID, $CycleID, '".$TransactionStart."', '".$TransactionEnd."', 3, ".$TrackID.")";
		
		//print $sql;
			if($mydb->sql_query($sql)){
				#update invoice detail cycle id
				$sql = "UPDATE tblCustomerInvoiceDetail SET BillingCycleID = $CycleID,StartBillingDate='".$TransactionStart."',EndBillingDate='".$TransactionEnd."' WHERE InvoiceID=$InvoiceID";
				$mydb->sql_query($sql);
		...
		function CreateInvoiceDetailMultiItem($CustomerID, $AccountID, $InvoiceID, $BillItemID, $Amount, $BilledUnit = 0, $IsVAT = 0, $TrackID = 0){
			global $mydb, $myinfo;
			...
			# Get Bill Description
			$tAmount=0;
			for($j=0; $j < count($BillItemID); $j++)
			{
				$sql = "SELECT ItemName, InvoiceSequence FROM tlkpInvoiceItem WHERE ItemID= ".$BillItemID[$j];
				$que = $mydb->sql_query($sql);
				$result = $mydb->sql_fetchrow();
				$Description = $result['ItemName'];
				$InvoiceSequence = $result['InvoiceSequence'];
				...
				//$Amount = floatval(FormatCurrency($Amount, "", 1, 2));
					...
					$sql = "INSERT INTO tblCustomerInvoiceDetail(InvoiceID, SequenceNo, BillItemID, Description, 
																BillUnit, Amount, IsVAT, AccID, CustID, TrackID) 
									VALUES(".$InvoiceID.", ".$InvoiceSequence.", ".$BillItemID[$j].", '".$Description."', 
								".$BilledUnit.", ".number_format($Amount[$j], 2, '.', '').", ".$IsVAT.", ".$AccountID.", ".$CustomerID.", ".$TrackID.")";
				$mydb->sql_query($sql);
				$mydb->sql_freeresult();
				$tAmount += $Amount[$j];
			}
			...
			{
				$VATRate = intval(getConfigue('VAT Rate'));
				$VATAmount = doubleval(($tAmount * $VATRate) / 100); 
		...
		//	Create Invoice
		//
		function CreateInvoice($CustomerID, $AccountID, $InvoiceID, $TransactionStart = "", $TransactionEnd = "", $TrackID = 0){
			global $mydb, $myinfo;
...
//								(select InvoiceID from tblCustCashDrawer where CustID = $CustomerID and (IsRollback is NULL or IsRollback = 0))";
		...
		function URDeposit($CustomerID, $AccountID, $ncDeposit, $icDeposit, $mfDeposit){
			...
			if($que = $mydb->sql_query($sql))
			{
				if($mydb->sql_numrows() <= 0)
				{
					$sql = "Insert into tblAccountBalance(CustID,AccID,Deposit,Credit,Outstanding,UnpaidDeposit) values($CustomerID, $AccountID,0,0,0,0)";
					$que = $mydb->sql_query($sql);
				}
			}
