						...
						<form name="fdrawerclose" method="post" action="./" onSubmit="return false;">
							<table border="0" cellpadding="3" cellspacing="0" align="left" width="100%" bgcolor="#feeac2">																
											...
											<?php
												...
												if($que){
													...
													while($rst = $mydb->sql_fetchrow($que)){	
														...
														print "<option value='".$DrawerID."' $select>$DrawerID - ".$FullName."</option>";
													}
												}
												$mydb->sql_freeresult();
											?>
								...
								<?php
												...
												$sql = "
																Declare @BillEndDate datetime, @PaymentDate datetime 
																Declare @Exists bit,@GrpID int
																set @Exists = 0
																select @BillEndDate=MIN(BillEndDate) from tblSysBillRunCycleInfo where BillProcessed=0 
																select @PaymentDate=Max(PaymentDate) from tblCustCashDrawer where ISNULL(IsRollback,0) = 0 

																select GroupServiceID,COUNT(*) am into __drawer from tblCustProduct P inner join tblTarPackage t on 
																P.PackageID=t.PackageID
																Inner JOin tlkpService s on t.ServiceID=s.ServiceID
																where StatusID=1
																group by GroupServiceID

																while(exists(select * from __drawer) and @Exists=0)
																Begin
																	select @GrpID=(select top 1 groupserviceID from __drawer)
																	if (Not Exists( select * from tblBillRunInfo where Status=1 and RunType=1 and CONVERT(varchar,endbilldate,112)=CONVERT(varchar,@BillEndDate,112) and GrpSID in (@GrpID))) 	
																		set @Exists=1
																	delete from __drawer where GroupServiceID=@GrpID
																End
								...
								?>														
								<tr> 				  
								<td colspan="3" align="center"><input type="submit" tabindex="10" name="btnSubmit" value="Submit" <?php print $sta;?> class="button" onclick="submitForm();" />
