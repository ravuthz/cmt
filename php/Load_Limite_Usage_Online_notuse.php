...
<?php
	...
	$sql = "select *,cast(cast((CurrentUsage-CreditBalance) as Numeric(30,2)) as varchar(50))  Payment
			...
			AmountClosed,IsNull(ul.Amount,0) existing,B.BillingCycleID CycleID,IsNull(ul.Status,0) sta	from tblCustomer C Inner Join tblCustproduct p on 
			C.CustID=P.CustID Inner Join tblTarPackage T on P.packageID=T.packageID Inner Join tblBillingSummaryTmp B on P.accID=B.accID Left Join tblCustCashDrawer adv
			 on P.accID=adv.AcctID and adv.IsSubmitted=0 and (adv.IsRollBack is Null or adv.IsRollBack=0) and adv.TransactionModeID in (3) Left Join tblAccDeposit Ad on
			  Ad.AccID=P.accID Left Join tblAccountBalance Ab on P.accID=Ab.accID Left Join (tblException E Inner Join tblExceptionDetail Ed on E.accID=Ed.accID and 
			  Remark='Exception') on p.accID=E.accID Left Join [tblSwitchUsageLimitStatus] ul on p.accID=ul.accID and B.BillingCycleID=ul.CycleID
			where p.custID in ($where) and (swstatus is null or swstatus in (1,2)) group by 
			Remark,UserName,SubScriptionName,P.accID,C.Telephone,TarName,ul.Status,ul.amount,B.BillingCycleID
			having (cast(($lamount/$w)*100 as Numeric(30,2))<=(Sum(IsNull(B.Amount,0))-Sum(IsNull(Ab.Credit,0))))) V1 Order by 
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){	
			...
			$Done = "<input type=Button Onclick=\"Javascript:doIt(".$AccID.",1,".$AmountClosed.",".$CycleID.",".$existing.",'Add or Update Some Account','".$AccID.'1'."','".$Dn."');\" ".$rtype." name=\"btnSubmit\" value=\"Done\" style=\"font-size:8px;\" ID='".$AccID.'1'."' >";
