...
<script language="javascript" type="text/javascript">
	...
	function ShowR(act,cID,aID,md,cst,UserName,ServiceID,sName,pID)
	{
		if(document.getElementById("sidd_"+aID) == null) ////////////////////// Check element exists or not
		{
	...
	function act(AccID)
	{
		...
		{
			frmActive.elements["Recon_"+AccID].disabled =  false;
		}
		else
		{
			frmActive.elements["Recon_"+AccID].disabled =  true; 
		}
	}
			...
			if($result = $mydb->sql_fetchrow($que)){				
				...
				switch($StatusID){
					...
					case 4:	# close
						...
						$stfg = $foreClose;
						$stwd = "Closed";
						$bl = "[<a href='#' onclick='javascript:doR(\"Reconnect\",".$CustomerID.",".$AccountID.",0,".$StatusID.",\"".$AccountName."\",".$ServiceID.",\"".str_replace(array('"','\''),'',$SubscriptionName)."\",".$pID.");' onmouseover='javascript:ShowR(\"Reconnect\",".$CustomerID.",".$AccountID.",0,".$StatusID.",\"".$AccountName."\",".$ServiceID.",\"".str_replace(array('"','\''),'',$SubscriptionName)."\",".$pID.");' >Rec & Change</a>]";												
						break;
				}
