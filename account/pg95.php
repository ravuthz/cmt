...
<script language="JavaScript" type="text/javascript">
	...
	function submitForm(){
		...
		if(faddcharge.BillID.selectedIndex < 0){
			alert("Please select charge item");
			faddcharge.BillID.focus();
			return;
			...
			faddcharge.txtCharge.value = faddcharge.BillID.options[faddcharge.BillID.selectedIndex].text;
...
<?php
	...
	# =============== Get customer header =====================
		...
		if(!empty($smt) && isset($smt) && ($smt == "save95")){		
			$Audit = new Audit();
			$Amount = FixQuotes($Amount);
			$Description = FixQuotes($Description);
			$BillID = FixQuotes($BillID);
			...
			if(!$error){
				$sql = "SELECT * FROM tblAccRecurringCharge 
								WHERE AccID=$AccountID and ItemID = $BillID and StatusID = 0 and CloseDate > getDate()";
				if($que = $mydb->sql_query($sql)){
				//	if($mydb->sql_fetchrow()){
						if($mydb->sql_numrows() > 0){
							$retOut = $myinfo->info("Item you are adding to charge is still opened. Please choose another item or modify the item in recrring charge summary");
						}else{
							
							if(trim($txtExpire)==""){
								$sql = "INSERT INTO tblAccRecurringCharge(AccID, RecChargeDes, RecChargeFee, ItemID, StatusID, CloseDate)
												VALUES($AccountID, '".$txtCharge."- ".$Description."', $Amount, $BillID, 0, NULL)";
							}else{
								$sql = "INSERT INTO tblAccRecurringCharge(AccID, RecChargeDes, RecChargeFee, ItemID, StatusID, CloseDate)
												VALUES($AccountID, '".$txtCharge."- ".$Description."', $Amount, $BillID, 0, '".$txtExpire."')";
							}
							if($mydb->sql_query($sql)){
								if(!empty($txtExpire))
									$expire = "expire on ".formatDate($txtExpire, 6);
								$Description = "Add recurring charge $txtCharge amount: ".FormatCurrency($Amount)." $expire. Description: $Description";
								$Audit->AddAudit($CustomerID, $AccountID, "Add recurring charge", $Description, $user['FullName'], 1, 3);
								redirect('./?CustomerID='.$CustomerID.'&AccountID='.$AccountID.'&pg=99');
							}else{					
								$error = $mydb->sql_error();
								$retOut = $myinfo->error("Failed add recurring charge.", $error['message']);
							}
					//	}
						}
				}else{
					$error = $mydb->sql_error();
					$retOut = $myinfo->error("Failed add get existing recurring charge.", $error['message']);
				}				
			}								
		}				
?>
	...
	<tr>
		...
		<td valign="top" align="left">
			<table border="0" cellpadding="2" cellspacing="0" class="formbg" align="left" width="100%">
									<tr>
										<td align="left" class="formtitle"><b>ADD RECURRING CHARGE
									...
									<tr>
										<td colspan="2">
											<form name="faddcharge" method="post" action="./" onSubmit="return false;">
												<table border="0" cellpadding="3" cellspacing="0" align="center" width="100%" bgcolor="#feeac2">													
													<tr>
														...
														<td align="left">
														<select name="BillID" class="boxenabled" tabindex="1">	
															<?php
																$sql = "select ItemID, ItemName from tlkpInvoiceItem where IsManual = 1";
																if($que = $mydb->sql_query($sql)){
													...
													<tr>
														...
														<td align="left">
															<input type="text" name="Amount" tabindex="2" class="boxenabled" size="20" value="0" />(Exclude VAT)
														</td>					
													</tr>
													<tr>
														<td align="left">Expiry date:</td>
														<td align="left">
															<input type="text" tabindex="3" name="txtExpire" class="boxenabled" size="27" maxlength="30" value="" onKeyUp="DateFormat(this,this.value,event,false,'2')" onBlur="DateFormat(this,this.value,event,true,'2')" />
											...
											<input type="hidden" name="pg" value="95" />
											<input type="hidden" name="smt" value="save95" />
											</form>	
										</td>
									</tr>			
								</table>
