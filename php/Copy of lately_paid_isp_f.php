...
<?php
	...
	$retOutH='<br><br><table border="1" cellpadding="3" cellspacing="0" align="center" width="100%" height="100%" id="audit3" class="sortable" style="border-collapse:collapse" bordercolor="#cccccc">
							...
							<thead>																									
								...
								<th align="center" width="12%" style="border-left:1px solid #999999; border-top:1px solid #999999;">Package</th>
	...
	//$sql = "select * from dbo.fn_latelypaid_1m_isp_f('".$cid."','".$w."') where status<>'Closed' order by remark desc,exception,state,del ,phone,CustName";
	
	$sql = "Exec spDebt2Month '$cid','ISP'";
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){	
			...
			$dnow=date('Y-m-d');			
			$Done = "<input type=Button Onclick=\"Javascript:doIt(".$InvoiceID.",'".$phone."','".$in.'1'."','".$in."',".$AccID.",'".$dnow."');\" ".$rtype." name=\"btnSubmit\" value=\"Done\" style=\"font-size:7px;\" ID='".$in.'1'."' >";
