<?php
	...
	*/
	if(isset($smt) && (!empty($smt)) && ($smt == "save800") &&  ($pg == 800)){
		...
		$txtSerial = FixQuotes($txtSerial);
		$txtSupplierName = FixQuotes($txtSupplierName);
		$selServiceID = FixQuotes($selServiceID);
		$sql = "INSERT INTO tlkpCPE(CPEName, ServiceID, Active, SupplierName, SerialNumber)
									VALUES('".$txtName."', '".$selServiceID."', 1, '".$txtSupplierName."', '".$txtSerial."')";
							...
							<tr>
							 <td valign="top">
								...
								<tr>
									...
									<td align="left">
										<input type="text" tabindex="1" name="txtSerial" class="boxenabled" size="50" maxlength="50" value="<?php print $txtSerial; ?>" />											
									</td>
								</tr>					 
								<tr>
					...
					<td align="left">
						<table border="1" cellpadding="3" width="100%" cellspacing="0" align="center" id="audit3" class="sortable" style="border-collapse:collapse" bordercolor="#aaaaaa">
							...
							<?php								
								$iLoop = 0;
								// Get cpe information
								$sql = "select c.CPEID, c.CPEName, c.Active, c.SupplierName, s.ServiceName, SerialNumber 
								...
								if($que = $mydb->sql_query($sql)){				
									while($result = $mydb->sql_fetchrow($que)){
										...
										print '<td class="'.$style.'" align="left">'.$SerialNumber.'</td>';
