...
<?php
	...
	$sql = "select * from dbo.fn_latelypaid_1m_delay('".$cid."','".$where."') where status<>'Closed' order by remark,status,state,del,expire,phone,custname";
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){	
			...
			if(intval($State)==1 || ($Month1<=5 and $Month2==0) || ($del==1)) 
				$rtype = "disabled";
			else 
				$rtype = "";	
			...
			$Done = "<input type=Button Onclick=\"Javascript:doIt(".$InvoiceID.",'".$phone."','".strval($InvoiceID).'1'."','".strval($in)."');\" ".$rtype." name=\"btnSubmit\" value=\"Delay\" style=\"font-size:8px;\" ID='".strval($in).'1'."' >";
