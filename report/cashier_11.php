			...
			if($pt == 1){
				$sql .= '
					...
					BEGIN CATCH
					END CATCH
					SELECT inf.BillEndDate, p.PaymentModelID, pm.PaymentMode,i.InvoiceID, COUNT(i.InvoiceID) "NoInv", convert(numeric(8, 2), 0.00) "DTB", 
				...
				FROM tblCustomerInvoice i(nolock), tblCashPayment p(nolock), tblSysBillRunCycleInfo inf(nolock), 
					tlkpPaymentMode pm, tblCustProduct a(nolock)
				WHERE p.InvoiceID = i.InvoiceID 
					...
					AND CONVERT(VARCHAR, inf.BillEndDate, 112) ="'.$cycle.'" AND i.InvoiceAmount>0
				GROUP BY i.InvoiceID,inf.BillEndDate, p.PaymentModelID, pm.PaymentMode							 
					-------------------------------------------------------------------------------------
					SELECT inf.BillEndDate,I.InvoiceID, p.PaymentModelID, 0 "NoInv", IsNULL(SUM(p.PaymentAmount), 0) "Paid"
					INTO #Pay
					...
					GROUP BY I.InvoiceID, inf.BillEndDate, p.PaymentModelID
					---------------------------------------------------------------------------------------
						UPDATE ReportCashier 
						SET ReportCashier.Paid = #Pay.Paid
						FROM #Pay
						WHERE ReportCashier.[Type] = 1
							AND ReportCashier.PaymentModelID = #Pay.PaymentModelID AND ReportCashier.InvoiceID=#Pay.InvoiceID
					--------------------------------------------------------------------------------------------	
						SELECT inf.BillEndDate,I.InvoiceID, IsNULL(SUM(p.PaymentAmount), 0) "CN", p.PaymentModelID
						INTO #CN
						FROM tblCustomerInvoice i(nolock), tblCashPayment p(nolock), tblSysBillRunCycleInfo inf(nolock), tblCustProduct a(nolock)
						WHERE p.InvoiceID = i.InvoiceID
							AND p.AcctID = a.AccID
					...
					---------------------------------------------------------------------------------------	
						...
						FROM #CN
						WHERE ReportCashier.[Type] = 1
							AND ReportCashier.PaymentModelID = #CN.PaymentModelID AND ReportCashier.InvoiceID = #CN.InvoiceID 
					-------------------------------------------------------------------------------------------------- 
					SELECT inf.BillEndDate,I.InvoiceID, IsNULL(SUM(p.PaymentAmount), 0) "WB", p.PaymentModelID
						INTO #WB
						FROM tblCustomerInvoice i(nolock), tblCashPayment p(nolock), tblSysBillRunCycleInfo inf(nolock), tblCustProduct a(nolock)
						WHERE p.InvoiceID = i.InvoiceID
							AND p.AcctID = a.AccID	
					...
					--------------------------------------------------------------------------------------------------------	
						...
						FROM #WB
						WHERE ReportCashier.[Type] = 1
							AND ReportCashier.PaymentModelID = #WB.PaymentModelID AND ReportCashier.InvoiceID = #WB.InvoiceID 
					------------------------------------------------------------------------------------------------------------	
						SELECT inf.BillEndDate,I.InvoiceID, IsNULL(SUM(p.PaymentAmount), 0) "SB", p.PaymentModelID
						INTO #SB
						FROM tblCustomerInvoice i(nolock), tblCashPayment p(nolock), tblSysBillRunCycleInfo inf(nolock), tblCustProduct a(nolock)
						WHERE p.InvoiceID = i.InvoiceID
							AND p.AcctID = a.AccID
					...
					-----------------------------------------------------------------------------------------------------------	
						...
						FROM #SB
						WHERE ReportCashier.[Type] = 1
							AND ReportCashier.PaymentModelID = #SB.PaymentModelID AND ReportCashier.InvoiceID = #SB.InvoiceID 
					----------------------------------------------------------------------------------------------------------
					Delete from ReportCashier where DTB+BD+AP+Paid++RD+RB+CN+WB+SB=0
				...
				$sql .= '
							...
							SELECT left(convert(varchar,p.PaymentDate,112),6) BillEndDate, p.PaymentModelID, pm.PaymentMode, COUNT(p.PaymentID) "NoInv", 
									convert(numeric(8, 2), 0.00) "DTB", 
									convert(numeric(8, 2), 0.00) "BD", convert(numeric(8, 2), 0.00) "AP", convert(numeric(8, 2), 0.00) "Paid", 
									convert(numeric(8, 2), 0.00) "RD", convert(numeric(8, 2), 0.00) "RB", convert(numeric(8, 2), 0.00) "CN", 
									convert(numeric(8, 2), 0.00) "WB", convert(numeric(8, 2), 0.00) "SB", 2 "Type",p.PaymentID
							INTO ReportCashier
							FROM tblCashPayment p(nolock), 
								tlkpPaymentMode pm, tblCustProduct a(nolock)
							WHERE p.PaymentModelID = pm.PaymentID
								AND p.AcctID = a.AccID
								AND CONVERT(VARCHAR, p.PaymentDate, 112) BETWEEN "' . FormatDate($FromDate, 4) . '" AND "' . FormatDate($ToDate, 4) . '"
								AND a.PackageID IN (' . $Package . ') AND p.PaymentAmount>0									
							GROUP BY left(convert(varchar,p.PaymentDate,112),6), p.PaymentModelID, pm.PaymentMode,p.PaymentID
							-----------------------------------------------------------------------------															
							SELECT IsNULL(SUM(p.PaymentAmount), 0) "DTB", p.PaymentModelID,p.PaymentID
							INTO #DTB
							...
							WHERE p.AcctID = a.AccID
								...
								AND CONVERT(VARCHAR, p.PaymentDate, 112) BETWEEN "' . FormatDate($FromDate, 4) . '" AND "' . FormatDate($ToDate, 4) . '"
								AND a.PackageID IN (' . $Package . ')
							GROUP BY p.PaymentModelID,p.PaymentID
							-------------------------------------------------------------------------------------
							UPDATE ReportCashier 
							SET ReportCashier.DTB = #DTB.DTB
							FROM #DTB
							WHERE ReportCashier.[Type] = 2
								AND ReportCashier.PaymentModelID = #DTB.PaymentModelID AND ReportCashier.PaymentID = #DTB.PaymentID
							---------------------------------------------------------------------------------------
							SELECT IsNULL(SUM(p.PaymentAmount), 0) "BD", p.PaymentModelID,p.PaymentID
							INTO #BD
							FROM tblCashPayment p(nolock), tblCustProduct a(nolock)
							WHERE p.AcctID = a.AccID
							...
							FROM #BD
							WHERE ReportCashier.[Type] = 2
								AND ReportCashier.PaymentModelID = #BD.PaymentModelID AND ReportCashier.PaymentID = #BD.PaymentID
							--------------------------------------------------------------------------------------------
							SELECT IsNULL(SUM(p.PaymentAmount), 0) "AP", p.PaymentModelID, p.PaymentID
							INTO #AP
							FROM tblCashPayment p(nolock), tblCustProduct a(nolock)
							WHERE p.AcctID = a.AccID
							...
							FROM #AP
							WHERE ReportCashier.[Type] = 2
								AND ReportCashier.PaymentModelID = #AP.PaymentModelID AND ReportCashier.PaymentID = #AP.PaymentID
							-----------------------------------------------------------------------------------------
							SELECT IsNULL(SUM(p.PaymentAmount), 0) "RD", p.PaymentModelID, p.PaymentID
							INTO #RD
							FROM tblCashPayment p(nolock), tblCustProduct a(nolock)
							WHERE p.AcctID = a.AccID
							...
							FROM #RD
							WHERE ReportCashier.[Type] = 2
								AND ReportCashier.PaymentModelID = #RD.PaymentModelID AND ReportCashier.PaymentID = #RD.PaymentID 
							----------------------------------------------------------------------------------------------
							SELECT IsNULL(SUM(p.PaymentAmount), 0) "RB", p.PaymentModelID, p.PaymentID
							INTO #RB
							FROM tblCashPayment p(nolock), tblCustProduct a(nolock)
							WHERE p.AcctID = a.AccID
							...
							FROM #RB
							WHERE ReportCashier.[Type] = 2
								AND ReportCashier.PaymentModelID = #RB.PaymentModelID AND ReportCashier.PaymentID = #RB.PaymentID
							----------------------------------------------------------------------------------------------								
							Delete from ReportCashier where DTB+BD+AP+Paid++RD+RB+CN+WB+SB=0
							BEGIN TRY								
								DROP TABLE #DTB
						...
						<td align="left">
							<table border="0" cellpadding="1" cellspacing="0" align="left" class="formbg" width="100%">		
								<tr>
									...
									<td align="right">
									[<a href="../export/cashier_11.php?st=<?php print $FromDate; ?>&et=<?php print $ToDate; ?>&where=<?php print $Package;?>&pt=<?php print $pt; ?>&cycle=<?php print $cycle; ?>">Download</a>]
									</td>
								</tr>
								<tr>
									<td colspan="2"> 
											...
											<?php
												if($que = $mydb->sql_query($sql)){
													...
													while($result = $mydb->sql_fetchrow($que)){
														...
														if(($total > 0) && ($Type == 1)){
															$link = "<a href=\"javascript:showReport2('".$FromDate."', '".$ToDate."', ".$pt.", '".FormatDate($BillEndDate, 4)."', '".$Package."', '".$PaymentModelID."', '".$paytype."');\">".FormatCurrency($total)."</a>";
														}else{
