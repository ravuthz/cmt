<?php
	...
	*/
	...
	$Audit=new Audit();
	if(isset($smt) && (!empty($smt)) && ($pg == 1100) && isset($allowsubmit) && $allowsubmit=="true"){
	# Begin transaction sign up messenger
		...
		$cmbSubAction=intval($cmbSubAction);
		if($op=="add"){
			$DiscDestID=GetNextID("CreditRuleLimit");
			if(CheckExistingCreditRuleLimit($txtRuleName)){
				$retOut=$myinfo->warning("The information already existed.");
			}else{
				$sql="";
				$sql.= "INSERT INTO tblCreditLimitRules(CredID, CredName,Description,Status) ".
					...
					while($i<$numrecord){
						$FromDur=intval($_POST["txtOverFrom".$i]);
						$ToDur=intval($_POST["txtOverTo".$i]);
						...
						$sql.="INSERT INTO tlkpCreditLimitRules (CredID,OverFrom,OverTo,Action,Description,Suplementary) ".
						    "VALUES ('".$DiscDestID."','".$FromDur."','".$ToDur."','$Action','$Description','$SubAction'); ";
				...
				if($result=$mydb->sql_query($sql)){	
						$retOut = $myinfo->info("Successfully add new credit rule limit.");
						$Audit->AddAudit(0,0,"Add New Credit Limit Rule","Add Credit Rule: $Description",$user["FullName"],1,17);
				}else{
					$error = $mydb->sql_error();
					$retOut = $myinfo->error("Failed to add new new credit rule limit.", $error['message']);
					$Audit->AddAudit(0,0,"Add New Credit Limit Rule",$Description,$user["FullName"],0,17);
				}
				$op="add";
				$txtRuleName="";
				$txtDescription="";
			...
			if(CheckExistingCreditRuleLimit($txtRuleName,true,$hid)){
				...
				$sql="";
				$sql.= "UPDATE tblCreditLimitRules SET CredName='$txtRuleName', Description='$txtDescription' Where CredID='".intval($hid)."'";
				
					$mydb->sql_freeresult($result);
					$DiscDestID=intval($hid);
					$i=0;
					
					$sql.="Delete from tlkpCreditLimitRules where CredID='".$DiscDestID."';";
					$i=0;
				...
				//echo $sql;
				if($result=$mydb->sql_query($sql)){			
						$retOut = $myinfo->info("Successfully update credit rule limit.");
						$Audit->AddAudit(0,0,"Update Credit Limit Rule","Update credit rule: $Description.",$user["FullName"],1,17);
				}else{
					$error = $mydb->error();
					$retOut = $myinfo->error("Failed to update credit rule limit.", $error['message']);
					$Audit->AddAudit(0,0,"Update Credit Limit Rule","Update credit rule: $Description.",$user["FullName"],0,17);
				}
				$op="add";
				$txtRuleName="";
				$txtDescription="";
		...
		if($op=="edit"){
			$sql="select * from tblCreditLimitRules WHERE CredID='".intval($hid)."'";
			...
			while($row=$mydb->sql_fetchrow($query)){
				...
				$sql2="select * from tlkpCreditLimitRules
						Where CredID='".intval($hid)."'";
				$result2=$mydb->sql_query($sql2);
				$j=0;
				while ($row2=$mydb->sql_fetchrow($result2)) {
						$_POST["txtOverFrom".$j]=intval($row2["OverFrom"]);
						$_POST["txtOverTo".$j]=intval($row2["OverTo"]);
			...
			$sql="update tblCreditLimitRules set Status='1' WHERE CredID='".$hid."'";
				if($mydb->sql_query($sql)){					
					$retOut = $myinfo->info("Successfully activate credit rule limit.");
					$Audit->AddAudit(0,0,"Activate Credit Limit Rule","Activate credit rule: ID: $hid",$user["FullName"],1,17);
				}else{
					$error = $mydb->sql_error();
					$retOut = $myinfo->error("Failed to activate credit rule limit.", $error['message']);
					$Audit->AddAudit(0,0,"Activate Credit Limit Rule","Activate credit rule: ID: $hid",$user["FullName"],0,17);
				}
				$op="add";
				$txtRuleName="";
				$txtDescription="";
			...
			$sql="update tblCreditLimitRules set Status='0' WHERE CredID='".$hid."'";
				if($mydb->sql_query($sql)){					
					$retOut = $myinfo->info("Successfully deactivate credit rule limit.");
					$Audit->AddAudit(0,0,"Deactivate Credit Limit Rule","Deactivate credit rule: ID: $hid",$user["FullName"],1,17);
				}else{
					$error = $mydb->sql_error();
					$retOut = $myinfo->error("Failed to deactivate credit rule limit.", $error['message']);
					$Audit->AddAudit(0,0,"Deactivate Credit Limit Rule","Deactivate credit rule: ID: $hid",$user["FullName"],0,17);
				}
				$op="add";
				$txtRuleName="";
				$txtDescription="";
...
<script language="javascript">
	...
	function Reset(){
		document.location.href="./?pg=1100";
	}
	...
	function CheckValidateInput(){
		...
		for(var i=0;i<frml.numrecord.value;i++){
			eval("mOverFrom=frml.txtOverFrom"+i);
			eval("mOverTo=frml.txtOverTo"+i);
			...
			if(Number(mOverTo.value)<0){
				alert("Invalid To Duration value.");
				mOverTo.focus();
				return false;
			}
			if(Number(mOverTo.value)<Number(mOverFrom.value) && Number(mOverTo.value)!=0){
	...
	function CheckValidate(){
		...
		for(var i=0;i<frml.numrecord.value-1;i++){
			...
			for(var j=i+1;j<frml.numrecord.value;j++){
				...
				eval("mOverFrom2=frml.txtOverFrom"+j);
				eval("mOverTo2=frml.txtOverTo"+j);
...
<table border="0" cellpadding="2" cellspacing="0" class="formbg"  width="611">
			   <tr>
				 <td width="645" height="18" align="left" class="formtitle"><strong>Credit Limit Rule </strong></td>
			   </tr>
			   <tr>
				 <td valign="top">
				   <table border="0" cellpadding="2" cellspacing="0" width="100%" height="100%" bgcolor="#feeac2">
					...
					<tr>
					 ...
					 <tr>
					  ...
					  <td width="21%" align="center"><strong>Over From % </strong></td>
					  <td width="23%" align="center"><strong>Over To % </strong></td>
					...
					while ($k<$numrecord) {
						...
						$txtOverFrom=$_POST["txtOverFrom".$k];
						$txtOverTo=$_POST["txtOverTo".$k];
					...
					<tr>
					  ...
					  <td align="center">
					    <input type="text" name="txtOverFrom<?php echo $newindex?>" value="<?php echo $txtOverFrom;?>"  size="6" />				      </td>
					  <td  align="center">
					    <input type="text" name="txtOverTo<?php echo $newindex?>" value="<?php echo $txtOverTo;?>" size="6"  />				      </td>
					  <td  align="center">
			  ...
			  <input type="hidden" name="pg" id="pg" value="1100">
	  ...
	  <div align="left">
					<table border="0" cellpadding="2" cellspacing="0" class="formbg" width="609">
									...
									<tr>
										<td align="left" colspan="2">
											<table border="1" cellpadding="3" cellspacing="0" width="100%" id="1"  class=""  bordercolor="#aaaaaa"  bgcolor="#EFEFEF"  style="border-collapse:collapse">
												...
												<?php
												$sql="select *  from tblCreditLimitRules
												...
												while($row=$mydb->sql_fetchrow($result)){
													...
													if($row['Status']==0){
														...
														echo "<td>";
														if(HasCreditRuleLimitDetail($ID)){
							...
							echo "							</tr>";
														...
														echo '	<td class="'.$rowclass.'" align="left">'.$RuleName.'</td>
														<td class="'.$rowclass.'" align="left">'.$Description.'</td>';
														
												  echo "<td class='".$rowclass."' ><a href=\"?pg=1100&amp;op=edit&amp;hid=".$ID."\"><img src='./images/Edit.gif' border='0'></a>&nbsp;<a href=\"javascript:ActionConfirmation(".$ID.",'".addslashes($RuleName)."')\"><img src=\"./images/Delete.gif\" border=\"0\"></a></td>";
													echo '</tr>';																
													}
													if(HasCreditRuleLimitDetail($ID)){
													...
													echo "<td></td>";						
													echo "<td colspan='3' >";
													echo GetCreditRuleLimitDetail($ID);
