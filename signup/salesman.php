...
*/
	...
	if(isset($smt) && (!empty($smt)) && ($pg == 22)){
	# Begin transaction sign up messenger
		#$mydb->mssql_begin_transaction();
			$sql = "INSERT INTO tlkpSalesman (Salutation, Name, Address, Country, Phone, Fax, Email, Passport, DOB, Remark)
		...
		if($mydb->sql_query($sql)){
			...
			$description = "add new salesman: Name: $selSalutation $txtName; DOB: $txtDOB; phone: $txtPhone; fax: $txtFax; email: $txtEmail; duplicate: $txtPassport; address: $txtAddress, $selCountry.";

			$audit->AddAudit("", "", $title, $description, $Operator, 1, 1);
			$retOut = $myinfo->info("Successfully sign up new salesman.");
		}else{			
			$error = $mydb->sql_error();
			$retOut = $myinfo->error("Failed to sign up new salesman", $error['message']);
		}
	}
?>
...
<table border="0" cellpadding="2" cellspacing="0" class="formbg" align="center" width="550">
			   <tr>
				 <td align="left" class="formtitle" height="18"><b>SIGN UP NEW SALESMAN</b></td>
			   </tr>
			   <tr>
				 <td valign="top">
			  ...
			  <input type="hidden" name="pg" id="pg" value="22">
