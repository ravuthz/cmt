<?php
	...
	include_once("lately_paid_isp_af_delay.php");	
?>
...
<script language="javascript" type="text/javascript">
	...
	function doReq(ci,wh){
				...
				url = "paid_load_isp_f.php?&cid="+ci+"&w="+wh+"&mt=" + new Date().getTime();
	...
	function doReq_af(ci,wh){	
				url1 = "lately_paid_isp_af_delay_f.php?&cid="+ci+"&w="+wh+"&mt=1" + new Date().getTime();
	...
	function doIt(invID,ph,Inv,inID,accID,dnow){
		...
		if(comm = prompt("Are you sure that you would like to do this "+ph+" ?\n Please enter your comment.","Rec. on " + dd)){
			url = "Ins_switch_comment_re_f.php?invoiceID="+invID+"&phone="+ph+"&remark="+comm+"&accid="+accID+"&dp="+dnow+"&mt=" + new Date().getTime();
			di="";
			DoAction(url);
	...
	function doAf(invID,ph,Inv,inID,accID,dnow){
		dd=+ new Date().getFullYear( ) + "/" + new Date().getMonth() + "/" + new Date().getDay() + " " + new Date().getHours()+ ":" + new Date().getMinutes()+ ":" + new Date().getSeconds() ;
		if(comm = prompt("Are you sure that you would like to do this "+ph+" ?\n Please enter your comment.","Rec. on " + dd)){
			url = "Ins_switch_comment_af_f.php?invoiceID="+invID+"&phone="+ph+"&remark="+comm+"&accid="+accID+"&dp="+dnow+"&mt=" + new Date().getTime();
			di="";
			DoAction(url);
			document.getElementById(inID).innerHTML=comm;
			document.getElementById(Inv).disabled=true;
