...
<script language="JavaScript" type="text/javascript">
	...
	function submitForm(){
		...
		if(Trim(Amount.value == "")){
			...
			alert("Percentage discount must be a number");
			Amount.focus();
			return;
			...
			Amount.focus();
			return;		
		}else if(Number(Trim(Amount.value)) > 100){
			alert("Percentage discount must be less or equal 100");
			Amount.focus();
			return;			
...
<?php
	...
	# =============== Get customer header =====================
		...
		if(!empty($smt) && isset($smt) && ($smt == "save104")){		
			$Audit = new Audit();
			...
			if(!$error){
				...
				//$sql = "SELECT DiscountRecurring FROM tblCustProduct WHERE AccID=	$AccountID";
//				if($que = $mydb->sql_query($sql)){
//					if($result = $mydb->sql_fetchrow($que)){
//						$curAdjust = $result['DiscountRecurring'];
//						if(is_null($curAdjust) || empty($curAdjust)) $curAdjust = 0;
//						$curAdjust += $txtAmount;
						$sql = "UPDATE tblCustProduct SET 
												CreditRePercent = '".$txtAmount."',
												CreditRePercentExpire = '".$txtExpire."'
						...
						if($mydb->sql_query($sql)){
							...
							$sql = "INSERT INTO tblHisAdjustment(CustomerID, AccountID, Amount, DisType, DiscountDate, 
											...
											VALUES($CustomerID, $AccountID, $txtAmount, 2, GetDate(), '".$txtExpire."', 
															'".$user['FullName']."', '".$txtDescription."')";
							if($mydb->sql_query($sql)){			
								if(!empty($txtExpire))
								$expire = "expire on ".formatDate($txtExpire, 6);
								$Description = "Add recurring credit adjustment ".FormatCurrency($txtAmount)." % $expire. $txtDescription";
...
//				}else{
//					$error = $mydb->sql_error();
//					$retOut = $myinfo->error("Failed to get current recurring credit adjustment.", $error['message']);
//				}	
			}											
		}				
?>
									...
									<tr>
										<td valign="top" colspan="2">
											...
											<table border="0" cellpadding="4" cellspacing="0" width="100%" height="100%" class="formbody" bordercolor="#aaaaaa">																								
												<tr>
													<td align="left" nowrap="nowrap">Percentage credit:</td>
													<td align="left">
											...
											<input type="hidden" name="smt" value="save104" />
											</form>
										</td>
									</tr>							
								</table>						
