...
<script language="JavaScript" type="text/javascript">
	...
	function ValidateForm(){
		if(feditcontact.txtDesignate.value == ""){
			alert("Please enter designate name");
			feditcontact.txtDesignate.focus();
			return;
...
<?php
	...
	//
	if(!empty($smt) && isset($smt) && ($smt == "savepg16")){
		$selSalutaion = FixQuotes($selSalutaion);
		$txtDesignate = FixQuotes($txtDesignate);
		...
		$sql = "UPDATE tblCustDesignate SET
							DesignateName = '".$txtDesignate."',
							DesignateNameKhmer = ".encodeUnicode($txtDesignateKhmer).",
						...
						WHERE DesignateID = $DesignateID
					 ";
		if($mydb->sql_query($sql)){
			$title = "Update customer designate";
			...
			$retOut = $myinfo->info("Successful save changed customer designate");
		}else{
			$error = $mydb->sql_error();
			$retOut = $myinfo->error("Failed to update customer designate.", $error['message']);
		}
	}

	# =============== Get customer information =====================
	$sql = "select DesignateName, Salutation, DOB, Address, SangkatID, KhanID, CityID, CountryID,
	convert(varbinary(500), AddressKhmer) AS AddressKhmer,
			convert(varbinary(500), DesignateNameKhmer) AS DesignateNameKhmer,
								NationalityID, OccupationID, Phone, Email, IdentityMode, IdentityData
					from tblCustDesignate where DesignateID = $DesignateID";

	if($que = $mydb->sql_query($sql)){
		if($rst = $mydb->sql_fetchrow($que)){
			$DesignateName = $rst['DesignateName'];
			$DesignateNameKhmer = $rst['DesignateNameKhmer'];
							...
							<td align="left" valign="top">
								<form name="feditcontact" method="post" action="./">
									<table border="0" cellpadding="2" cellspacing="0" class="formbg" align="left" width="100%">
										<tr>
											<td align="left" class="formtitle" height="18"><b>Edit customer designate</b></td>
										</tr>
										<tr>
											<td valign="top">
												<table border="0" cellpadding="2" cellspacing="0" width="100%" height="100%" bgcolor="#feeac2">
													...
													<tr>
														...
														<td align="left" colspan="3">
															<input type="text" name="txtDesignate" class="boxenabled" size="83" maxlength="50" value="<?php print $DesignateName;?>" />
														</td>
													</tr>
													<tr>
														<td align="left"><span class="khfont">ឈ្មោះ:</span></td>
														<td align="left" colspan="3">
															<input type="text" name="txtDesignateKhmer" class="boxenabled khfont" size="83" maxlength="50" value="<?php print decodeUnicode($DesignateNameKhmer);?>" />
														</td>
													</tr>
													<tr>
									...
									<input type="hidden" name="pg" id="pg" value="16" />
									<input type="hidden" name="CustomerID" value="<?php print $CustomerID; ?>" />
									<input type="hidden" name="DesignateID" value="<?php print $DesignateID; ?>" />
									...
									<input type="hidden" name="smt" value="savepg16" />
								</form>
							</td>
						</tr>
					</table>
