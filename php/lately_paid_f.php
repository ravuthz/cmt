...
<script language="javascript" type="text/javascript">
	...
	function doIt(invID,ph,Inv,inID,acid,dnow){
		dd= new Date().getFullYear( ) + "/" + new Date().getMonth() + "/" + new Date().getDay() + " " + new Date().getHours()+ ":" + new Date().getMinutes()+ ":" + new Date().getSeconds() ;
		if(comm = prompt("Are you sure that you would like to do this "+ph+" ?\n Please enter your comment.","dis. on " + dd)){
			url = "Ins_switch_comment_f.php?invoiceID="+invID+"&phone="+ph+"&remark="+comm+"&dp="+dnow+"&ai="+acid+"&mt=" + new Date().getTime();
			di="";
			DoAction(url);
...
<?php
	...
	$retOutH='<br><br><table border="1" cellpadding="3" cellspacing="0" align="center" width="100%" height="100%" id="audit3" class="sortable" style="border-collapse:collapse" bordercolor="#cccccc">
							...
							<thead>																									
								...
								<th align="center" width="5%" style="border-left:1px solid #999999; border-top:1px solid #999999;" nowrape>Mark</th>
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){	
			...
			$retOut .= '<td class="'.$style.'" align="right" style="border-left:1px solid #999999; border-top:1px solid #999999;">'.$delay.'</td>';
