...
<script language="JavaScript" type="text/javascript">
	...
	function ValidateForm(){
		if(feditcontact.txtContact.value == ""){
			alert("Please enter contact name");
			feditcontact.txtContact.focus();
			return;
			...
			return;
		}else if(feditcontact.comment.value == ""){
			alert("Pleas enter comment for change");
...
<?php
	...
	//
	if(!empty($smt) && isset($smt) && ($smt == "savepg15")){
		$selSalutaion = FixQuotes($selSalutaion);
		$txtContact = FixQuotes($txtContact);
		...
		$sql = "UPDATE tblCustContact SET
							ContactName = '".$txtContact."',
							ContactNameKhmer = ".encodeUnicode($txtContactKhmer).",
							Salutation = '".$selSalutaion."',
							DOB = '".$txtDOB."',
							Address = '".$txtAddress."',
							AddressKhmer = ".encodeUnicode($txtAddressKhmer).",
		...
		if($mydb->sql_query($sql)){
			...
			//$description = "Change contact info as: Name: $selSalutaion $txtContact; DOB: $txtDOB; nationality: $txtNationality; occupation: $txtOccupation; phone: $txtPhone; email: $txtEmail; duplicate: $selDuplicateID- $txtDuplicate; address: $txtAddress; sangkat: $txtSangkat; Khan: $txtKhan; City: $txtCity; Country: $txtCountry.";
			$description = $comment;
			$audit->AddAudit($CustomerID, "", $title, $description, $Operator, 1, 7);
			$retOut = $myinfo->info("Successful save changed customer contact");
	...
	$sql = "select ContactName, Salutation, DOB, Address, SangkatID, KhanID, CityID, CountryID,
			convert(varbinary(500), AddressKhmer) AS AddressKhmer,
			convert(varbinary(500), ContactNameKhmer) AS ContactNameKhmer,
								NationalityID, OccupationID, Phone, Email, IdentityMode, IdentityData
					from tblCustContact where ContactID = $ContactID";

	if($que = $mydb->sql_query($sql)){
		if($rst = $mydb->sql_fetchrow($que)){
			$ContactName = $rst['ContactName'];
			$ContactNameKhmer = $rst['ContactNameKhmer'];
							...
							<td align="left" valign="top">
								<form name="feditcontact" method="post" action="./">
									<table border="0" cellpadding="2" cellspacing="0" class="formbg" align="left" width="100%">
										<tr>
											<td align="left" class="formtitle" height="18"><b>Edit customer contact</b></td>
										</tr>
										<tr>
											<td valign="top">
												<table border="0" cellpadding="2" cellspacing="0" width="100%" height="100%" bgcolor="#feeac2">
													...
													<tr>
														...
														<td align="left" colspan="3">
															<input type="text" name="txtContact" class="boxenabled" size="83" maxlength="50" value="<?php print $ContactName;?>" />
														</td>
													</tr>
													<tr>
														<td align="left"><span class="khfont">ឈ្មោះ:</span></td>
														<td align="left" colspan="3">
															<input type="text" name="txtContactKhmer" class="boxenabled khfont" size="83" maxlength="50" value="<?php print decodeUnicode($ContactNameKhmer);?>" />
														</td>
													</tr>
													<tr>
											...
											<button class="invisibleButtons" onClick="window.open( './javascript/calendar.html?feditcontact|txtDOB', '', 'width=200,height=220,top=250,left=350');">
												<img src='./images/b_calendar.png' alt='View Calendar' align="middle" border="0">
													...
													<tr>
														...
														<td align="left" colspan="3">
															<input type="text" name="txtAddressKhmer" class="boxenabled khfont" tabindex="1" size="83" maxlength="100" value="<?php print decodeUnicode($AddressKhmer); ?>" />
														</td>
													</tr>
													<tr>
									...
									<input type="hidden" name="pg" id="pg" value="15" />
									<input type="hidden" name="CustomerID" value="<?php print $CustomerID; ?>" />
									<input type="hidden" name="ContactID" value="<?php print $ContactID; ?>" />
									...
									<input type="hidden" name="smt" value="savepg15" />
								</form>
							</td>
						</tr>
					</table>
