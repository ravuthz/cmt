...
<script language="JavaScript" type="text/javascript">
	...
	function submitForm(){
		...
		if(foneadjust.txtMailBox.value == ""){
			...
			return;
		}else if(!isNumber(foneadjust.txtMailBox.value)){
			alert("Please enter valid value of mailbox quality");
			foneadjust.txtMailBox.focus();
			return;
		}
...
<?php
	...
	# =============== Get customer header =====================
		...
		if(!empty($smt) && isset($smt) && ($smt == "save198")){		
		
			$Audit = new Audit();
			$txtMailBox = intval(FixQuotes($txtMailBox));
			...
			if($op == "add"){
				...
				$error = false;			
				$sql2 = "select * from dbo.tblIspAccAdditionalMail
							where accid = $AccountID
						";
								
				$rownum=$mydb->sql_query($sql2);
				if($mydb->sql_numrows($rownum)>0){
					$error = true;				
					$retOut = $myinfo->error("Faill to add additional mail box.", "additional mail box information was overlapsed with other.");	
				}
				if(!$error){
					# Get current adjustment				
							if(trim($txtEndDate)==""){
									$sql = "INSERT INTO tblIspAccAdditionalMail
	(AccID, MailBox, EffectiveDate, EndDate, PerMailBoxCharge)
	
	VALUES ($AccountID, '$txtMailBox', '$txtEffectiveDate', NULL, $txtPerMailBoxCharge)";
								}else{
									$sql = "INSERT INTO tblIspAccAdditionalMail
	(AccID, MailBox, EffectiveDate, EndDate, PerMailBoxCharge)
	
	VALUES ($AccountID, '$txtMailBox', '$txtEffectiveDate', '$txtEndDate', $txtPerMailBoxCharge)";
								}						
		
							if($mydb->sql_query($sql)){
								$Audit->AddAudit($CustomerID, $AccountID, "Additional mail box", $Description, $user['FullName'], 1, 12);
								redirect('./?CustomerID='.$CustomerID.'&AccountID='.$AccountID.'&pg=198');
								
							}else{	
								$error = $mydb->sql_error();
								$retOut = $myinfo->error("Failed to add additional mail box.", $error['message']);				
							}									
				}		
			}elseif($op=="edit"){
				$error = false;			
				if(!$error){
					# Get current adjustment				
							if(trim($txtEndDate)==""){
$sql = "UPDATE tblIspAccAdditionalMail
SET AccID = $AccountID, MailBox = '$txtMailBox', EffectiveDate = '$txtEffectiveDate', EndDate = NULL,		PerMailBoxCharge = $txtPerMailBoxCharge
WHERE MailBoxID = $mailboxid";
							}else{
								$sql = "UPDATE tblIspAccAdditionalMail
SET AccID = $AccountID, MailBox = '$txtMailBox', EffectiveDate = '$txtEffectiveDate', EndDate = '$txtEndDate', PerMailBoxCharge = $txtPerMailBoxCharge
WHERE MailBoxID = $mailboxid";
							}						
		
							if($mydb->sql_query($sql)){
								$Audit->AddAudit($CustomerID, $AccountID, "Additional mail box", "Add new additional mail box for : account $AccountID", $user['FullName'], 1, 12);
								redirect('./?CustomerID='.$CustomerID.'&AccountID='.$AccountID.'&pg=198');
								
							}else{	
								$error = $mydb->sql_error();
								$retOut = $myinfo->error("Failed to update additional mail box.", $error['message']);				
							}									
					}		
				}									
		}		
		elseif (isset($op) && $op == "edit" && isset($mailboxid)){
			$sql = "select * from tblIspAccAdditionalMail 
	...
	<tr>
		...
		<td valign="top" align="left">
			<table border="0" cellpadding="2" cellspacing="0" class="formbg" align="left" width="100%">
									...
									<tr>
									 	<td align="left" class="formtitle"><b>Additional Mail Box - <?php print $aSubscriptionName." (".$aUserName.")"; ?></b>										</td>
										<td align="right"></td>
									</tr>
									<tr>
										<td valign="top" colspan="2">
											...
											<table border="0" cellpadding="4" cellspacing="0" width="100%" height="100%" class="formbody" bordercolor="#aaaaaa">																								
												<tr>
													<td width="18%" align="left" nowrap="nowrap">Number of mail box:</td>
													<td width="82%" align="left"><input type="text" size="27" name="txtMailBox" class="boxenabled" value="<?php print($txtMailBox);?>" />												     
												  &nbsp;</td>
												</tr>
						...
						?>"   type="text" size="25" /><a href="javascript:NewCal('txtEndDate','yyyymmdd',true,12)"><img src="images/cal.gif" width="16" height="16" border="0" alt="Pick a date"></a></td>
											...
											<input type="hidden" name="pg" value="198" />
											...
											<input type="hidden" name="smt" value="save198" />
											</form>										</td>
									</tr>		
									<tr>
									 	<td align="left" class="formtitle"><b>Additional Mail Box Information</b>										</td>
										<td align="right"></td>
									</tr>
									<tr>
									  ...
									  <table width="100%" border="1"  bordercolor="#666666" style="border-collapse:collapse" bgcolor="#FFFFFF">
										...
										<?php
											$sql = "select * from tblIspAccAdditionalMail where AccID = $AccountID";
											$query=$mydb->sql_query($sql);
											while($row=$mydb->sql_fetchrow($query)){
                                        ...
                                        <tr>
									      ...
									      <td><?php echo "<a href='?CustomerID=".$CustomerID."&amp;AccountID=".$AccountID."&amp;pg=198&amp;op=edit&amp;mailboxid=".$MailBoxID."'><img src='./images/Edit.gif' border='0'></a>";?></td>
                                        </tr>
										  <?php 
