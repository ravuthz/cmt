...
$sql = " SELECT InvoiceType, Count(*) 'Inv', Sum(Duration) 'duration', SUM(OtherCharge) 'Other',
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){																															
			...
			if(($iLoop % 2) == 0)											
				...
				$style = "row2";
			$Inv = "<a href='../report/revenue_detail_group_loc_pack.php?sid=".$sid."&st=".$cid."&it=".$InvoiceType."&pid=".$pid."&pt=".$pt."&tt=".$tt."&tid=".$tid."&pack=".$pack."' target='_blank'>".$Inv."</a>";
			$retOut .= '<tr>';																																													
		...
		$mydb->sql_freeresult();
		$TotalInv = "<a href='../report/revenue_detail_group_loc_pack.php?sid=".$sid."&st=".$cid."&it=All&pid=".$pid."&pt=".$pt."&tt=".$tt."&tid=".$tid."&pack=".$pack."' target='_blank'>".$TotalInv."</a>";
		$retOut .= '</tbody>
	...
	$retfun = "<table border=0 cellpadding=0 cellspacing=0 width='100%'>
						...
						";
		$sql = "select Distinct GrpServiceName , TarName GrpPackage, PackageID from tblLocRev
						where Convert(varchar,BillEndDate,112) = '".$cid."'
						and GroupServiceID = ".$sid."
						and PackageID in ( ".$where." ) 
						and cityid in ( ".$tid." )
						";
		$sql .= " order by GrpPackage ";	
	...
	if($que = $mydb->sql_query($sql)){
		while($result = $mydb->sql_fetchrow($que)){
			...
			$PackageID = $result['PackageID'];
				...
				$retfun .= "<tr><td>";
				$retfun .= generateReport($cid, $PackageID, $tid, $ct, $GrpPackage,$tt, $GrpServiceName, $where, $sid);
				$retfun .= "</td></tr>";
		}
	}
