...
function generateReport($cid, $pid, $tid, $ct, $pt, $tt, $sid){
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){																															
			...
			if(($iLoop % 2) == 0)											
				...
				$style = "row2";
			$Inv = "<a href='../report/revenue_detail_group.php?sid=".$sid."&st=".$cid."&it=".$InvoiceTypeID."&pid=".$pid."&pt=".$pt."&tt=".$itype."' target='_blank'>".$Inv."</a>";
			$retOut .= '<tr>';																																													
		...
		$mydb->sql_freeresult();
		$TotalInv = "<a href='../report/revenue_detail_group.php?st=".$cid."&it=0&pid=".$pid."&pt=".$pt."&tt=All&sid=".$sid."' target='_blank'>".$TotalInv."</a>";
		$retOut .= '</tbody>
	...
	$retfun = "<table border=0 cellpadding=0 cellspacing=0 width='100%'>
						...
						";
		$sql = "SELECT Distinct PackageID = Case
				When PackageID in (16,17 ) then 'in (16,17)'
				when PackageID in (18) then 'in (18)'
				when PackageID in (19) then 'in (19)'
				else 'not in (16,17,18,19)'
				end, TarName = case
				When PackageID in (16,17 ) then 'PCO'
				when PackageID in (18) then 'INTERNAL'
				when PackageID in (19) then 'PAYPHONE'
				else 'CUSTOMER'
				END, ServiceID 	
...
//		$sql .= "	AND ServiceID = ".$sid;	
	...
	$retfun .= "<tr><td>";
		$retfun .= generateReport($cid, "in (".$where.")", $tid, $ct, "GRAND TOTAL", $tt, $ServiceID);
	$retfun .= "</td></tr>";
	$retfun .= "</table>";
	print $retfun;	
