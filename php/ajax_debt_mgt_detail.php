...
<?php		
		...
		$sql = "SELECT ci.InvoiceID, Convert(varchar,ci.IssueDate,102) IssueDate, ci.InvoiceAmount, ci.NetAmount, ci.VATAmount, ci.UnpaidAmount, cp.CustID, cp.AccID,
		 								cp.UserName, left(cp.SubscriptionName,15) SubscriptionName, cp.StatusID,IsNull(ex.Remark,'Nor') Remark,left(tp.TarName,7) TarName
						...
						WHERE convert(varchar, sb.BillEndDate, 102) = '".$code."'";
		...
		if($k == "p")
		$sql .= "and ci.InvoiceAmount - ci.UnpaidAmount > 0 ";
		else if($k == "u")
		$sql .= "and ci.UnpaidAmount > 0 ";
