	 ...
	 **/	
		...
		function FormatSecond($second)
		{
			...
			if(($second%3600)/60 < 10)
				{
					$h .= "0".floor(($second%3600)/60).":";
				}
			else
				{
					$h .= floor(($second%3600)/60).":";
				}
				
				$h .= substr("0".floor(($second%3600)%60),-2);
		 ...
		 * Format currency
		 */
		function FormatCurrency($Amount, $currency = "\$", $type = 1, $ap = 2){
			# 100 cents
			if($type == 1)
				$Amount = $Amount;
			# 10 cents
			...
			elseif($type == 3)
				$Amount = $Amount / 100;
			return $currency.number_format($Amount, $ap);
		}
