...
<?php
	...
	$sql = "select * from dbo.fn_latelypaid_1m_request('2007-08-31','2007-09-18') where convert(char(10),pdate,120)>='2007-10-01' and status<>'Closed' order by ty,exception desc,remark desc ,state,del,pdate,phone,CustName";
	...
	if($que = $mydb->sql_query($sql)){				
		...
		while($result = $mydb->sql_fetchrow($que)){	
			...
			if(intval($State)==1 || $Exception=='VIP' || $Remark=='Active' ) //|| $del==1) 
				$rtype = "disabled";
			else 
				$rtype = "";	
