...
function generateReport($cid, $pid, $tid, $ct, $pt, $tt, $sname, $pack, $sid){
	...
	$cmd = "select Distinct Upper(Location) Location, CityID from tblLocRev
			where Convert(varchar,BillEndDate,112) = '".$cid."'
		...
		while($result1 = $mydb->sql_fetchrow($que1)){
			...
			$retfun .= "<tr><td align=center class=formtitle><b>".$Location."</b></td></tr>";	
				$sql = "select Distinct GrpServiceName , GrpPackage from tblLocRev
						where Convert(varchar,BillEndDate,112) = '".$cid."'
						and GroupServiceID = '".$sid."'
				...
				if($que = $mydb->sql_query($sql)){
					while($result = $mydb->sql_fetchrow($que)){
						$GrpServiceName = $result['GrpServiceName'];
						$GrpPackage = $result['GrpPackage'];
							$retfun .= "<tr><td>";
							$retfun .= generateReport($cid, $GrpPackage, $CityID, $ct, $GrpPackage, $Location, $GrpServiceName, $where, $sid);
							$retfun .= "</td></tr>";
					}
				}
				$retfun .= "<tr><td>";
					$retfun .= generateReport($cid, "GRAND TOTAL", $CityID, $ct, "GRAND TOTAL", $Location, $GrpServiceName, $where, $sid);
				$retfun .= "</td></tr>";
				...
				if($que2 = $mydb->sql_query($sql2)){
					while($result2 = $mydb->sql_fetchrow($que2)){
						...
						$GrpPackage = $result2['GrpPackage'];
							$retfun .= "<tr><td>";
							$retfun .= generateReport($cid, $GrpPackage, $tid, $ct, $GrpPackage, "All", $GrpServiceName, $where, $sid);
							$retfun .= "</td></tr>";
					}
				}
			
				$retfun .= "<tr><td>";
					$retfun .= generateReport($cid, "GRAND TOTAL", $tid, $ct, "GRAND TOTAL", "All", $GrpServiceName, $where, $sid);
				$retfun .= "</td></tr>";
	}
